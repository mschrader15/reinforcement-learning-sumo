...
def find_matching_param(
    ...
    """Helper function to find a xmltodict parameter that contains the given
    key and value.

    Args:
        params (List[OrderedDict]): a list of xmltodict ordered dictionaries
            that all have the same tag
...
class DualRingActor(_Base):
    ...
    def current_state(self, state_list: Union[list, Set[int]]) -> None:
        self._current_state = set(state_list)
    ...
    def _build(self, nema_config_xml: str, net_file_xml: str) -> None:
        ...
        # find the order that they control and then the "controllng" detectors
        for phase in tl_dict["phase"]:
            ...
            for lane_index in controlled_lane_index:
                # try to find a matching parameter in the traffic light configuration 
                # file
        ...
        bs = [
            find_matching_param(
                tl_dict["param"], attr="@key", value="barrierPhases"
            ).get("@value", "")
        ]
        # b2 can either be called barrier2Phases or coordinatePhases
        bs.append(
            find_matching_param(
                tl_dict["param"], attr="@key", value="barrier2Phases"
            ).get("@value", "")
        )
        bs[-1] = (
            bs[-1]
            if bs[-1] != ""
                ...
                tl_dict["param"], attr="@key", value="coordinatePhases"
            ).get("@value", "")
        )
    ...
    def intialize_control(
        ...
        """Take control of all of the NEMA lights by overriding the detection
        to 0, meaning that the light will not be actuated by simulation
        traffic."""
    ...
    def try_switch(self, requested_state: List[int]) -> None:
        ...
        Args:
            requested_state (List[int, int]): a list of requested states.
                Should likely have length 2 but there could be longer or
                shorter scenarios
        """
    ...
    def get_state_representation(
        ...
        """Get the state representation of the traffic light.

        Returns:
            int: the state representation
        """
        state = self.get_actual_state()
        return self._action_space.index(state)
...
class GlobalDualRingActor:
    ...
    def update_lights(self, action_list: list) -> None:
        ...
        for action, tl_manager in zip(action_list, self.tls):
            tl_manager.try_switch(tl_manager.action_space[action])
