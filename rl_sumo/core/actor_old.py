...
class LightHead(Observable, _Base):
    ...
    def __init__(self, name, phase, phase_info, initial_state):
        ...
        self._timers = {_TL_HEAD.RED: 0.0, _TL_HEAD.YELLOW: 0.0, _TL_HEAD.GREEN: 0.0, _TL_HEAD.YIELD: 0.0}
        self._minimum_times = {
    ...
    def transition(
        ...
        if desired == self.state:
            return True
        if (desired in self.ALLOWED_PROGRESSIONS[self.state]) or force:
            if self._check_timer() or force:
                self.state = desired
    ...
    def process_observations(self, *args, **kwargs):
        ...
        elif main_color == _TL_HEAD.YELLOW and self.state != _TL_HEAD.YELLOW:
            self.state = _TL_HEAD.YELLOW
